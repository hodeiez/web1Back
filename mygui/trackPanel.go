...
func TrackPanel() *fyne.Container {
	...
	but := widget.NewButton("button", func() {
		data.Set([]string{titleInp.Text, descrInp.Text, dateInp.Text, fileRefInp.Text})
		setData, _ := data.Get()
	...
	return container.New(layout.NewGridLayout(2), container.New(layout.NewFormLayout(), label1, titleInp, label2, descrInp, label3, dateInp, label4, fileRefInp, label5, but), result)
}
